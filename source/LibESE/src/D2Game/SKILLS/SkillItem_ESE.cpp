...
//D2Game.0x6FD02BA0
int32_t __fastcall ESE_SKILLITEM_pSpell01_Initializer(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
...
//D2Game.0x6FD02BF0
int32_t __fastcall ESE_SKILLITEM_pSpell01_IdentifyItem(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
...
//D2Game.0x6FD02C80
int32_t __fastcall ESE_SKILLITEM_pSpell02_CastPortal(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    if (pUnit && pUnit->dwUnitType == UNIT_PLAYER)
    {
        ...
        if (!DUNGEON_IsRoomInTown(pRoom))
        {
            ...
            const int32_t nResult = ESE_D2GAME_CreatePortalObject_6FD13DF0(pGame, pUnit, pRoom, CLIENTS_GetUnitX(pUnit), CLIENTS_GetUnitY(pUnit), nTownLevelId, &pPortal, 0x3Bu, 0);
...
//D2Game.0x6FD02E10
int32_t __fastcall ESE_SKILLITEM_pSpell03_Potion(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    if (!pItem)
    {
        return 0;
    ...
    for (int32_t i = 0; i < 3; ++i)
    {
        ...
        D2ItemStatCostTxt* pItemStatCostTxtRecord = ESE_SKILLS_GetItemStatCostTxtRecord(nStatId);
        ...
        if (nAddValue > 0)
        {
            ...
            if (nStatId == STAT_HITPOINTS || nStatId == STAT_HPREGEN)
            {
                if (pUnit && pUnit->dwUnitType == UNIT_PLAYER)
                {
                    nAddValue = ESE_sub_6FD0F8B0(pGame, pUnit, nAddValue);
                }
            }
...
//D2Game.0x6FD032D0
int32_t __fastcall ESE_SKILLITEM_pSpell04_Unused(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    if (!pItem)
    {
        return 0;
...
//D2Game.0x6FD03610
int32_t __fastcall ESE_SKILLITEM_pSpell05_RejuvPotion(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    if (!pItem)
    {
        return 0;
...
//D2Game.0x6FD039A0
int32_t __fastcall ESE_SKILLITEM_pSpell09_StaminaPotion(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    if (!pItem)
    {
        return 0;
    ...
    ESE_sub_6FD11C90(pUnit, pItemsTxtRecord->wState, 0);
    ...
    for (int32_t i = 0; i < 3; ++i)
    {
        if (ESE_SKILLS_GetItemStatCostTxtRecord(pItemsTxtRecord->wStat[i]))
        {
...
//D2Game.0x6FD03BB0
int32_t __fastcall ESE_SKILLITEM_pSpell09_AntidoteThawingPotion(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    ...
    if (pItemsTxtRecord)
    {
        ...
        if (ESE_SKILLITEM_pSpell09_StaminaPotion(pGame, pUnit, pItem, pTarget, nX, nY, nSkillId))
        {
            nResult = 1;
        }
...
//D2Game.0x6FD03C80
int32_t __fastcall ESE_SKILLITEM_pSpell08_ExperienceElixir(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
...
//D2Game.0x6FD03CD0
int32_t __fastcall ESE_SKILLITEM_pSpell07_OpenCube(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    if (!pUnit || pUnit->dwUnitType != UNIT_PLAYER)
    {
        return 0;
...
//D2Game.0x6FD03D80
int32_t __fastcall ESE_SKILLITEM_pSpell10_CastFireBallOnTarget(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    if (DATATBLS_GetItemsTxtRecord(pItem ? pItem->dwClassId : -1))
    {
        ...
        if (pTarget)
        {
            return ESE_SKILLITEM_CastSkillOnTarget(pUnit, SKILL_FIREBALL, 1, pTarget, 0);
        }
        else
        {
...
//D2Game.0x6FD03DF0
int32_t __fastcall ESE_SKILLITEM_pSpell11_CastFireBallToCoordinates(D2GameStrc* pGame, D2UnitStrc* pUnit, D2UnitStrc* pItem, D2UnitStrc* pTarget, int32_t nX, int32_t nY, int32_t nSkillId)
{
    if (DATATBLS_GetItemsTxtRecord(pItem ? pItem->dwClassId : -1))
    {
        return ESE_SKILLITEM_CastSkillOnPosition(pUnit, SKILL_FIREBALL, 1, nX, nY, 0);
    }
...
//D2Game.0x6FD040B0
int32_t __fastcall ESE_SKILLS_SrvDo113_Scroll_Book(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t nSkillId, int32_t nSkillLevel)
{
    if (!pUnit->pInventory)
    {
        return 0;
...
//D2Game.0x6FD04270
void __fastcall ESE_SKILLITEM_ActivateAura(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t a5, int32_t nSkillId, int32_t nSkillLevel)
{
    if (!pUnit)
    {
        return;
    ...
    EVENT_SetEvent(pGame, pUnit, UNITEVENTCALLBACK_PERIODICSTATS, ESE_sub_6FD13220(pGame, pUnit, pSkillsTxtRecord, nSkillId, nSkillLevel), a5, nSkillId);

    if (pSkillsTxtRecord->dwFlags[0] & gdwBitMasks[SKILLSFLAGINDEX_IMMEDIATE])
    {
        ESE_D2GAME_SKILLS_Handler_6FD12BA0(pGame, pUnit, nSkillId, nSkillLevel, 1, 1, 0);
    }
}

//D2Game.0x6FD04340
void __fastcall ESE_SKILLITEM_DeactivateAura(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t a4, int32_t nSkillId)
{
    if (!pUnit)
    {
        return;
...
int32_t __fastcall ESE_SKILLITEM_EventFunc09_Stupidity(D2GameStrc* pGame, int32_t nEvent, D2UnitStrc* pAttacker, D2UnitStrc* pUnit, D2DamageStrc* pDamage, int32_t nSkillId, int32_t nSkillLevel)
{
    ...
    if (nBaseValue > 0)
    {
        ...
        ESE_sub_6FD0B9F0(pGame, pAttacker, pUnit, nSkillLevel);
        return 1;
    }
...
//D2Game.0x6FD04C40
int32_t __fastcall ESE_SKILLITEM_CalculateOpenWoundsHpRegen(int32_t nLevel, const int32_t* pValues)
{
    if (nLevel <= 1 || !pValues)
    {
        return 0;
...
int32_t __fastcall ESE_SKILLITEM_EventFunc15_OpenWounds(D2GameStrc* pGame, int32_t nEvent, D2UnitStrc* pAttacker, D2UnitStrc* pUnit, D2DamageStrc* pDamage, int32_t nSkillId, int32_t nSkillLevel)
{
    ...
    int32_t nHpRegen = ESE_SKILLITEM_CalculateOpenWoundsHpRegen(nLevel, nValues) + 40;
...
int32_t __fastcall ESE_SKILLITEM_EventFunc20_SkillOnAttackHitKill(D2GameStrc* pGame, int32_t nEvent, D2UnitStrc* pAttacker, D2UnitStrc* pUnit, D2DamageStrc* pDamage, int32_t nSkillId, int32_t nSkillLevel)
{
    ...
    D2SkillsTxt* pSkillsTxtRecord = ESE_SKILLS_GetSkillsTxtRecord(v10);
    if (!pSkillsTxtRecord)
    {
        return 0;
    }
    ...
    if (pSkillsTxtRecord->dwFlags[0] & gdwBitMasks[SKILLSFLAGINDEX_ITEMTGTDO])
    {
        if (pUnit)
        {
            ESE_SKILLITEM_CastSkillOnTarget(pUnit, v10, v9, pUnit, 0);
            return 1;
        }
    }
    else
    {
        if (pUnit)
        {
            ESE_SKILLITEM_CastSkillOnTarget(pAttacker, v10, v9, pUnit, 1);
            return 1;
        }
...
int32_t __fastcall ESE_SKILLITEM_EventFunc21_SkillOnGetHit(D2GameStrc* pGame, int32_t nEvent, D2UnitStrc* pAttacker, D2UnitStrc* pUnit, D2DamageStrc* pDamage, int32_t nSkillId, int32_t nSkillLevel)
{
    ...
    if (pAttacker)
    {
        ...
        if (nChance > 0)
        {
            ...
            {
                if (ESE_SKILLS_GetSkillsTxtRecord(v10))
                {
                    if (pUnit)
                    {
                        ESE_SKILLITEM_CastSkillOnTarget(pAttacker, v10, v9, pUnit, 0);
                    }
                    else
                    {
...
//D2Game.0x6FD05C20
int32_t __fastcall ESE_SKILLITEM_FindTargetPosition(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t nSkillId, int32_t nSkillLevel)
{
    if (!pUnit)
    {
        return 0;
...
//D2Game.0x6FD05D70
int32_t __fastcall ESE_SKILLITEM_HandleItemEffectSkill(D2GameStrc* pGame, D2UnitStrc* pUnit, int32_t nSkillId, int32_t nSkillLevel, D2UnitStrc* pTargetUnit, int32_t nX, int32_t nY, int32_t* pUnitType, int32_t* pUnitGUID, int32_t* pX, int32_t* pY, int32_t a7)
{
    D2SkillsTxt* pSkillsTxtRecord = ESE_SKILLS_GetSkillsTxtRecord(nSkillId);
    ...
    case SKILLSITEMTARGET_CASTER:
    {
        ...
        {
            nResult = ESE_D2GAME_SKILLS_Handler_6FD12BA0(pGame, pUnit, nSkillId, nSkillLevel, 0, 1, a7);
        }
        break;
    }
    case SKILLSITEMTARGET_RANDOM:
    {
        if (ESE_SKILLITEM_FindTargetPosition(pGame, pUnit, nSkillId, nSkillLevel))
        {
            if (pX)
            {
    ...
    {
        D2UnitStrc* pTarget = ESE_sub_6FD15210(pUnit, SUNIT_GetTargetUnit(pGame, pUnit), nSkillId, nSkillLevel);
        if (pTarget)
        {
...
//D2Game.0x6FD06020
int32_t __fastcall ESE_SKILLITEM_CastSkillOnTarget(D2UnitStrc* pUnit, int32_t nSkillId, int32_t nSkillLevel, D2UnitStrc* pTargetUnit, int32_t a5)
{
    ...
    if (!ESE_SKILLITEM_HandleItemEffectSkill(pUnit->pGame, pUnit, nSkillId, nSkillLevel, pTargetUnit, 0, 0, &nTargetUnitType, &nTargetUnitGUID, &nTargetX, &nTargetY, a5))
    {
        return 0;
    }
...
//D2Game.0x6FD060F0
int32_t __fastcall ESE_SKILLITEM_CastSkillOnPosition(D2UnitStrc* pUnit, int32_t nSkillId, int32_t nSkillLevel, int32_t nX, int32_t nY, int32_t a0)
{
    ...
    if (!ESE_SKILLITEM_HandleItemEffectSkill(pUnit->pGame, pUnit, nSkillId, nSkillLevel, 0, nX, nY, &nTargetUnitType, &nTargetUnitGUID, &nTargetX, &nTargetY, a0))
    {
        return 0;
    }
